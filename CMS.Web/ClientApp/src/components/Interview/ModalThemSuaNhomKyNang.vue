      ...
      <v-card-text class="px-4 py-0">
        <v-row>
          <v-col cols="12" class="mt-0 pb-0">
            <v-text-field
              v-model="nhomKyNang.tenNhomKyNang"
              :error-messages="errors.collect('Tên nhóm kỹ năng', 'frmAddEdit')"
              v-validate="'required'"
              data-vv-scope="frmAddEdit"
              data-vv-name="Tên nhóm kỹ năng"
    ...
    save(): void {
      this.$validator.validateAll("frmAddEdit").then((res) => {
        if (res) {
          this.loading = true;
          if (this.isUpdate) {
            NhomKyNangApi.updateNhomKyNang(this.nhomKyNang.id, this.nhomKyNang)
              .then((res) => {
                this.loading = false;
            ...
            NhomKyNangApi.createNhomKyNang(this.nhomKyNang)
              .then((res) => {
                this.loading = false;
